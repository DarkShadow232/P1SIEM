  ...
  return (
    ...
    >
      <div className="relative max-w-5xl mx-auto px-6 lg:px-8">
        ...
        {/* Chart Container */}
        <div
          className={`bg-[#0a0e27] border border-[#8c52ff]/10 p-8 transition-all duration-700 delay-200 ${
            isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
          }`}
        >
          {/* Legend */}
          <div className="flex flex-wrap gap-6 mb-8 justify-center">
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-gradient-to-r from-[#4d238b] to-[#8c52ff]" />
              <span className="text-sm text-[#d9d9d9]/70">Supervised</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-4 h-4 bg-gradient-to-r from-[#8e50ec]/60 to-[#8c52ff]/60" />
              <span className="text-sm text-[#d9d9d9]/70">Unsupervised</span>
            </div>
          </div>
              ...
              return (
                <div
                  ...
                  className={`group transition-all duration-300 ${
                    hoveredBar === model.id ? 'scale-[1.02]' : ''
                  }`}
                ...
                >
                  ...
                  <div className="flex justify-between items-center mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      {isHighest && (
                        <span className="px-2 py-0.5 bg-[#8c52ff] text-white text-xs font-bold">
                          BEST
                        </span>
                      )}
                    </div>
                    <span className="text-lg font-bold text-[#8c52ff]">
                      {model.accuracy}%
                    </span>
                  </div>

                  {/* Bar */}
                  <div className="h-10 bg-[#121212] overflow-hidden relative">
                    <div
                    ...
                    {[90, 95, 100].map((mark) => (
                      <div
                        key={mark}
                        className="absolute top-0 bottom-0 w-px bg-[#8c52ff]/20"
                        style={{ left: `${mark}%` }}
                      />
                    ))}
                  </div>
                  ...
                  {/* Type Label */}
                  <div className="mt-1 text-xs text-[#d9d9d9]/50">
                    {model.type}
                  </div>
                </div>
              );
          ...
          {/* X-Axis Labels */}
          <div className="flex justify-between mt-6 pt-4 border-t border-[#8c52ff]/10">
            <span className="text-xs text-[#d9d9d9]/50">0%</span>
            <span className="text-xs text-[#d9d9d9]/50">50%</span>
            <span className="text-xs text-[#d9d9d9]/50">100%</span>
          </div>
        </div>
      </div>
    </section>
