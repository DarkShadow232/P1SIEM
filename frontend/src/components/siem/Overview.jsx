    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 xl:gap-20 items-start lg:items-center">
          ...
          >
            ...
            <div className="mt-6 sm:mt-8 flex flex-wrap gap-2 sm:gap-3 md:gap-4">
              {['Zero-Day Detection', 'Real-Time Analysis', 'Enterprise-Ready'].map((feature, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-[#8c52ff]/5 border border-[#8c52ff]/20"
                >
                  <div className="w-1.5 h-1.5 bg-[#8c52ff] rounded-full flex-shrink-0" />
          ...
          {/* Stats Grid */}
          <div
            className={`grid grid-cols-2 gap-3 sm:gap-4 transition-all duration-700 delay-400 ${
              isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'
            }`}
          >
            {statsData.map((stat, index) => {
              ...
              return (
                <div
                  key={index}
                  className="group relative p-4 sm:p-5 md:p-6 bg-[#121212] border border-[#8c52ff]/10 hover:border-[#8c52ff]/40 transition-all duration-400 hover:-translate-y-1"
                >
                  ...
                  <div className="relative">
                    <Icon className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 text-[#8c52ff]/70 mb-3 sm:mb-4 group-hover:text-[#8c52ff] transition-colors" />
                    <div className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-1 sm:mb-2">
                      {stat.value}
                    </div>
                    <div className="text-sm sm:text-base text-[#8c52ff] font-medium mb-0.5 sm:mb-1">
                      {stat.label}
                    </div>
                    <div className="text-xs sm:text-sm text-[#d9d9d9]/60">
                      {stat.description}
                    </div>
                  </div>
                </div>
