...
import { Database, Settings, Brain, AlertCircle, Server, Monitor, ArrowRight } from 'lucide-react';
              ...
              {methodologyData.map((step, index) => {
                ...
                return (
                  ...
                  >
                    {/* Step Card */}
                    <div
                      className={`relative p-6 bg-[#0a0e27] border transition-all duration-500 cursor-pointer ${
                        isActive
                    ...
                    >
                      {/* Step Number */}
                      <div
                        className={`absolute -top-3 left-1/2 -translate-x-1/2 w-6 h-6 flex items-center justify-center text-xs font-bold transition-all duration-500 ${
                          isActive
                      ...
                      {/* Content */}
                      <h3
                        className={`text-sm font-bold text-center mb-2 transition-colors duration-500 ${
                          isActive ? 'text-[#8c52ff]' : 'text-white'
                        }`}
                      >
                        {step.title}
                      </h3>
                      <p className="text-xs text-[#d9d9d9]/60 text-center">
                        {step.description}
                      </p>
                    </div>
          ...
          <div className="lg:hidden space-y-4">
            {methodologyData.map((step, index) => {
              ...
              return (
                <div
                  key={step.step}
                  className={`relative p-6 bg-[#0a0e27] border transition-all duration-500 ${
                    isActive
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex flex-col items-center">
                      <div
                        className={`w-8 h-8 flex items-center justify-center text-sm font-bold mb-2 ${
                          isActive
                            ? 'bg-[#8c52ff] text-white'
                      ...
                      <Icon className={`w-6 h-6 ${isActive ? 'text-[#8c52ff]' : 'text-[#8c52ff]/50'}`} />
                    </div>

                    {/* Content */}
                    <div className="flex-1">
                      <h3 className={`font-bold mb-1 ${isActive ? 'text-[#8c52ff]' : 'text-white'}`}>
                        {step.title}
                      </h3>
